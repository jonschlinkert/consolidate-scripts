...
var removeStyles = function(str, type) {
  var $ = cheerio.load(str);

  if (type === 'src') {
    $('style').filter(function(i, el) {
      return $(this).attr('src') !== undefined;
    }).remove();
  } else if (type === 'block') {
    $('style').filter(function(i, el) {
...
var extractStyleBlock = function(str) {
  var $ = cheerio.load(str);
  var styles = [];

  $('style').filter(function(i, el) {
    styles.push($.html($(this)));
  });

  return _.compact(styles);
};

var consolidateStyleBlocks = function(str) {
  var stylesArray = extractStyleBlock(str).join('\n');
  var sanitized = removeStyles(str, 'block');

  var $ = cheerio.load(sanitized);
  var placeholder = $.html($('style#insert'));
  var html = $.html();

  return html.replace(placeholder, stylesArray);
};


file.writeFileSync('test/actual/index.html', consolidateStyleBlocks(str));
